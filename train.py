...
map_characters ={0: 'Batman', 1: 'GreenLantern', 2: 'Spiderman',3: 'WonderWomen'}
...
def load_pictures(BGR):
    ...
    for k, char in map_characters.items():
        pp="/home/kapitsa/PycharmProjects/cartoon/simpsonRecognition/SimpsonRecognition/"
        pictures = [k for k in glob.glob(pp+'/characters/%s/*' % char)]
...
def create_model_six_conv(input_shape):
    ...
    """
    model = Sequential()
    model.add(Conv2D(1024, (3, 3), padding='same', input_shape=input_shape))
    model.add(Activation('relu'))
...
def training(model, X_train, X_test, y_train, y_test, data_augmentation=True):
    ...
    if data_augmentation:
        ...
        datagen.fit(X_train)
        path="/home/kapitsa/PycharmProjects/cartoon/simpsonRecognition/SimpsonRecognition/model//"
        filepath=path+"weights_6conv_%s.hdf5" % time.strftime("%d%m_%Y")
